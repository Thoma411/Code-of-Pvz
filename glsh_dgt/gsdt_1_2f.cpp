 ...
 * @LastEditTime: 2022-11-21 22:40:06
 */
#include <iostream>
#include <ctime>
#include <vector>
...
int main()
{
    ...
    char dfName[fName_sufLen * 2], cfName[fName_sufLen]; //输出文件名=默认+自定义后缀.txt
    ...
    vector<pair<int, int>> eatTime_HPArr;                        //存活啃食时间与损血表
    eatTime_HPArr.clear();
    ...
    for (int ti = 0; ti < test_times; ti++) //测试次数
    {
        ...
        {
            countEat++;
            int canEat = gametick - digger_eat;
            int eatHP = (canEat - 1) / EI * EI + EI; //**需要注意的是当前输出的文本文件中, 啃食时间1现在对应矿工存活时间353, 所以伤害为4
            // eatTime_HPArr[ti].first = gametick; //添加单次测试结束时间(array)
            eatTime_HPArr.push_back(make_pair(canEat, eatHP)); // 353对应啃食时刻1
        }
        printf("test num: %d\n", ti + 1);
    ...
    fpt = fopen(dfName, "w+");
    for (int it = 0; it < eatTime_HPArr.size(); it++)
        fprintf(fpt, "%d\t%d\n", eatTime_HPArr[it].first, eatTime_HPArr[it].second);
    fclose(fpt);
